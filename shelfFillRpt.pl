...
my $annexSQL = "SELECT shelf, size, tray, COUNT(item) AS \"items\", SUM(thickness) AS \"width\", SUM(thickness) / 8::float AS \"inches\", AVG(thickness) AS \"avg\", MAX(thickness) AS \"max\", MIN(thickness) AS \"min\" ".
    "FROM ( ".
...
foreach my $t (sort {$a->{'shelf'} cmp $b->{'shelf'} || $a->{'tray'} cmp $b->{'tray'}} @$data) {
    my %tray = %$t;
    
    next if ($tray{'tray'} =~ /^TRAY-$tray{'shelf'}/);
    if ($tray{'width'} < ($tray{'size'} =~ /^E/ ? 69 : 105)) {
        if ($tray{'shelf'} =~ /^SHELF-\w-\d{3}-\w$/ && $tray{'tray'} =~ /^TRAY-\w\w\d{5}$/) {
            $rpt .= join(' | ', (sprintf('%-13s', $tray{'shelf'}), sprintf('%-4s', $tray{'size'}), sprintf('%-12s', $tray{'tray'}), sprintf('%5d', $tray{'items'}), sprintf('%5d', $tray{'width'}), sprintf('%7.2f', 100 * ($tray{'width'} / ($tray{'size'} =~ /^E/ ? 92 : 140)))))."\n";
            #~ $table .= "    <tr>\n".join ('', map("        <td>$_</td>\n", ($tray{'shelf'}, $tray{'size'}, $tray{'tray'}, $tray{'items'}, $tray{'inches'}, 100 * ($tray{'width'} / ($tray{'size'} =~ /^E/ ? 92 : 140)))))."    \n</tr>\n";
            $table .= "    <tr>\n";
            ...
            $table .= "        <td style=\"text-align: right;\">".sprintf('%.2f', 100 * ($tray{'width'} / ($tray{'size'} =~ /^E/ ? 92 : 140)))."</td>\n";
            $table .= "    </tr>\n";
        } else {
            $erpt .= join(' | ', (sprintf('%-40s', $tray{'shelf'}), sprintf('%-4s', $tray{'size'}), sprintf('%-40s', $tray{'tray'}), sprintf('%5d', $tray{'items'}), sprintf('%5d', $tray{'width'}), sprintf('%7.2f', 100 * ($tray{'width'} / ($tray{'size'} =~ /^E/ ? 92 : 140)))))."\n";
            #~ $etable .= "    <tr>\n".join ('', map("        <td>$_</td>\n", ($tray{'shelf'}, $tray{'size'}, $tray{'tray'}, $tray{'items'}, $tray{'inches'}, 100 * ($tray{'width'} / ($tray{'size'} =~ /^E/ ? 92 : 140)))))."    </tr>\n";
            $etable .= "    <tr>\n";
            ...
            $etable .= "        <td style=\"text-align: right;\">".sprintf('%.2f', 100 * ($tray{'width'} / ($tray{'size'} =~ /^E/ ? 92 : 140)))."</td>\n";
            $etable .= "    </tr>\n";
        }
        say $fh join(',', ($tray{'shelf'}, $tray{'size'}, $tray{'tray'}, $tray{'items'}, $tray{'width'}, $tray{'inches'}, 100 * ($tray{'width'} / ($tray{'size'} =~ /^E/ ? 92 : 140)), $tray{'avg'}, $tray{'max'}, $tray{'min'}));
    }
}
close($fh);
